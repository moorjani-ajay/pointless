...
// Optional TLS for hosted Postgres (e.g. RDS). Local/compose leaves this unset.
...
export async function getPresentationByToken(token: string): Promise<Presentation | null> {
  const { rows } = await pool.query<DeckRow>('SELECT * FROM decks WHERE share_token = $1', [token]);
  return rows[0] ? toFull(rows[0]) : null;
}
...
export async function setHtml(id: string, html: string, title?: string): Promise<boolean> {
  const { rows } = await pool.query<{ title: string }>('SELECT title FROM decks WHERE id = $1', [id]);
...
export async function publishPresentation(
  ...
  if (passwordHash === undefined) {
    await pool.query('UPDATE decks SET published = true, updated_at = $1 WHERE id = $2', [now(), id]);
  } else {
    await pool.query(
